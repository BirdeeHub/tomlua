{
  ...
  outputs = {self, nixpkgs}: let
    ...
    , ...
      ...
      mkdir -p "$out"
      $CC -O3 -fPIC -flto -shared -o "$out/${APPNAME}.so" '${./src/tomlua.c}' '${./src/parse_str.c}' '${./src/parse_keys.c}' '${./src/parse.c}' -I'${lua}/include'
    '';
  in {
    ...
    devShells = forAllSys (system: let
      ...
      devbuild = pkgs.writeShellScriptBin "build_cc" ''
        mkdir -p ./build
        ${pkgs.bear}/bin/bear -- $CC -O3 -fPIC -flto -shared -o ./build/tomlua.so ./src/tomlua.c ./src/parse_str.c ./src/parse_keys.c ./src/parse.c -I"${lua}/include"
      '';
    in {
