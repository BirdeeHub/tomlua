...
static inline bool parse_key(lua_State *L, str_iter *src, str_buf *buf, bool int_keys) {
    ...
    if (!current.ok) {
        set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 32, "expected key, got end of content");
        return false;
    }
    char c = current.v;
    ...
    if (c == '"') {
        ...
        while (is_identifier_char(current.v)) {
            if (!buf_push(buf, current.v)) {
                set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 3, "OOM");
                return false;
            }
            iter_skip(src);
            current = iter_peek(src);
...
static int parse_keys(lua_State *L, str_iter *src, str_buf *buf, bool int_keys) {
    ...
    while (iter_peek(src).ok) {
        ...
        if (consume_whitespace_to_line(src)) {
            set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 33, "newlines not allowed between keys");
            return false;
        }
        ...
        if (consume_whitespace_to_line(src)) {
            set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 68, "newlines not allowed between keys and their terminators: =, ], or ]]");
            return false;
        }
        ...
        if (next.ok) {
            if (next.v == '=' || next.v == ']') {
                ...
                set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 43, "keys may not contain spaces without quotes!");
                return false;
            }
        } else {
            set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 13, "trailing key!");
            return false;
        }
    }
    if (!i) set_tmlerr(new_tmlerr(L, DECODE_DEFINED_IDX), false, 14, "empty key list");
    return i;
}
